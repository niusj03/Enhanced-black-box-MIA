...
# Queue paper-style black-box SimMIA runs and generate the expensive caches.
#
...
#
#   mkdir -p logs
#   nohup bash scripts/run_paper_simmia_cache_queue.sh "0 1 2 3 4 5 6 7" \
...
run_simmia_combo() {
  ...
  for method in $METHODS; do
    ...
    run_command "$method--$data--$sub--$model" "${WRAPPER_CMD[@]}"
  done
}
...
run_samia_combo() {
  ...
  run_command "samia--$data--$sub--$model" "${WRAPPER_CMD[@]}"
}
