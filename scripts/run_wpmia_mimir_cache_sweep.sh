...
# Sweep WPMIA tau/gamma over complete MIMIR caches under simmia_out/mimir.
...
#
# Useful overrides:
#   CACHE_ROOT="simmia_out/mimir"
#   WPMIA_TAUS="0.03 0.05 0.1 0.2 0.5"
#   WPMIA_GAMMAS="0 0.25 0.5 0.75 1.0"
#   CSV_ROOT="logs/wpmia"
#   RUN_ROOT="logs/wpmia/$(date +%Y%m%d_%H%M%S)_mimir_wpmia"
#   CONTINUE_ON_ERROR=1
#   DRY_RUN=1
...
RUN_ROOT="${RUN_ROOT:-logs/wpmia/$(date +%Y%m%d_%H%M%S)_mimir_wpmia}"
...
csv_for_group() {
  local split="$1"
  local model="$2"
  printf '%s/mimir_%s_%s_wpmia_sweep.csv' \
    "$CSV_ROOT" "$(slugify "$split")" "$(slugify "$model")"
}
...
append_csv_row() {
  ...
  python - "$csv_path" "$status" "$DATA" "$split" "$sub_dataset" "$model" \
...
parse_mimir_cache_path() {
  local records="$1"
  local rel
  rel="${records#simmia_out/mimir/}"
...
run_one_setting() {
  ...
  if [[ "${sub_dataset,,}" == "dm_mathematics" ]]; then
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    append_csv_row "$csv_path" "dry_run" "$split" "$sub_dataset" "$model" \
      "$num_shots" "$tau" "$gamma" "$result_name" "$output_dir" "$roc_path" \
      "$log_file"
  ...
  append_csv_row "$csv_path" "$status" "$split" "$sub_dataset" "$model" \
    "$num_shots" "$tau" "$gamma" "$result_name" "$output_dir" "$roc_path" \
    "$log_file"
...
log "MIMIR cache root: $CACHE_ROOT"
log "Tau values: $TAUS"
log "Gamma values: $GAMMAS"

if [[ ! -d "$CACHE_ROOT" ]]; then
  log "ERROR missing MIMIR cache root: $CACHE_ROOT"
  exit 2
fi

total_caches=0
while IFS= read -r records_path; do
  parse_mimir_cache_path "$records_path" || {
    log "SKIP unrecognized MIMIR cache path: $records_path"
    continue
  }

  cache_dir="$(dirname "$records_path")"
  csv_path="$(csv_for_group "$SPLIT" "$MODEL")"
  scenario_slug="$(slugify "mimir_${SPLIT}_${MODEL}_${SUB_DATASET}_${NUM_SHOTS}")"

  if ! cache_check_output="$(check_cache_complete "$cache_dir" 2>&1)"; then
    log "SKIP incomplete cache: $cache_dir"
    log "$cache_check_output"
    append_csv_row "$csv_path" "skipped_incomplete_cache" "$SPLIT" \
      "$SUB_DATASET" "$MODEL" "$NUM_SHOTS" "" "" "" "$cache_dir" "" \
      "$SUMMARY_LOG"
    continue
  fi
  ...
  for tau in $TAUS; do
    for gamma in $GAMMAS; do
      run_one_setting "$cache_dir" "$SPLIT" "$SUB_DATASET" "$MODEL" \
        "$NUM_SHOTS" "$tau" "$gamma" "$csv_path" "$scenario_slug"
    done
  done
done < <(find "$CACHE_ROOT" -path '*/records.jsonl' -print | sort)

log "Finished. Swept $total_caches complete MIMIR caches."
